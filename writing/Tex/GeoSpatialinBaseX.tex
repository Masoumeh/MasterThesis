...
Storage and retrieval of XML-based geospatial data in native XML databases are also suitable ways to fulfill the geospatial requirements.

Our contribution is to add geospatial functionalities in the native XML database \textit{BaseX}~\cite{www/basex}.
Toward this goal, we support geospatial data in \emph{Geography Markup Language (GML)} format
and existing libraries are used to do the spatial calculations on GML.
The big challenge is the effectiveness of implementation
since geospatial data is often huge in size and complex in structure.
Hence, performance improvement of data retrieval from a huge data structure 
needs a second thought. 
Spatial indexing structures are common solutions 
to find the suitable ways of efficient querying according to specific constraints.
Then, we need to find a specific indexing structure that fulfills our requirements.
We have followed this purpose throught the following tasks,
\begin{itemize}
...
Since the number of indexing structures are large for the first task, 
we selected the structures among the most widely accepted ones in the literature.
The goal in this task is to add a new source of discussion 
...
we also provide a short summary explaining the indexing structures, advantages, and disadvantages.
This summary helps to decide the optimum structure for our approach. 

To gain an insight into the different ways of implementation of geospatial functionalities,
some other database systems with the geospatial ability are practical samples.
We study these selective databases in the second task.
We expalin how the geospatial functionlities are provided in each database.
Furthermore, the indexing structures together with different possibilites 
are discussed briefly.

In the next steps, we develope the Geo Module together with the indexing structure in BaseX.
Evaluation of this module using the real-world data
comes hereafter to see the efficiecny of the implementation.
...
\subsection{Summary}
\label{indexsummary}
In this section, we explained some important spatial indexing trees which are widely studied in the literature. Indeed, there is a huge number of indexing structures introduced in various sources over this area. These structures are categorized differently in each source. However an extensive research on indexing trees was not our focus, we tried to cover a set of leading ones as an introduction in developing a proper index structure in BaseX. As mentioned before, choosing an index depends on the characterisitc of the system and also the functionalities that the system provides. Of course the spatial characteristic of data should be considered as well. 
Regarding the discussion in this section, there is no optimum indexing structure for any type of data and requirements. Based on the application, each structure has own pros and cons. Together with the explanations, we also provided a brief description of advantages and disadvantages for each index. %structure is briefly mentioned based on the studies and experiments.% in Chin et al~\cite{survey} and Brown et al~\cite{tree-basedindexes} together with. 
...
Based on the requirements, one can search this table for the propert tree of that special case.

Among the presented indexes, the STR-tree is the one which we have selected for further implementation. 
We had different reasons for this selection.
In general, trees based on B-tree are designed to search efficiently structures stored in disk files.
According to Lee et al~\cite{Lee}, R-trees are a common technique which is widely used in spatial databases.
They have mostly one hundered percent space utilization.
Another advantage of STR-tree is the preprocessing data which reduces the query time later in the execution time.
Also, the STR-tree as a static variant of the R-tree is counted as an advanced one 
and is effective for datasets with few changes, which we have started with.
Papadopoulos~\ref{Papadopoulos2010} states that STR-tree generally outperforms
the previously proposed Bulk-loading methods except some minor cases.
Besides, this structure is included in the JTS library, a widely used open source API that is used in BaseX implementing the Geo Module. Since the number of open source libraries which implements these tree structures is small and we did not aim to implement a structure from the begining, the STR-tree package in the JTS API which is compatible with BaseX Geo Module is a good option to choose.

Another tree structure which could be considered for the implementation is Quadtree.  
It is also included in the JTS library. 
In contrast to STR-tree, it is particularly optimized when the data set changes dynamically.
...
Up to this point we have tried various ways to decrease the query times in the Geo Module. Further attempts could be done following the aforementioned Java profiling list (Table~\ref{javaprofiling}), if the implementation will be continued in the current direction.
...
One of the first ideas is to bring some parts of the tree into RAM which are addresses more.
Regarding the indexing, an interesting future work is to compare 
