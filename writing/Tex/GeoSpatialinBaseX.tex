...
According to \emph{Collins} dictionary, the word \emph{geospatial} is an adjective relating to a position of things on the Earth's surface, also \emph{geospatial data} is the data representing any thing on the Earth's surface. Regarding the importance of the geospatial data, many standards and systems have been developed  during the years to manipukate, represent and make use of them. Different countries and organizations are using different systems and standards to represent geospatial data and consequently there are numerous systems to fulfill a range of requirements. The World Geodesic System (WGS) is one of the standard geospatial systems that is commonly used all around the world. WGS84 is the latest version of this standard which
...
In this section, we mainly explain some existing geospatial indexing structure as the most critical part of geospatial data processing in database systems. After some basic definitions in Section \ref{termsanddef}, general process of this data in the native databases is shortly discussed in Section \ref{s.geo-data-in-xml-data}.
