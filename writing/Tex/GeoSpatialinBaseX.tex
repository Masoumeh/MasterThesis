...
Figure~\ref{figkdtree} shows an example of a data and the corresponding KD-tree. As it can be seen, according to the discriminator in first level, $x$, all nodes in the left subtree of $A$ have smaller $x$ value and all nodes in the right subtree have greater $x$ values. In the next level, the discriinator is $y$ value and all left subtree nodes have smaller $y$ value while the right subtree nodes have greater $y$ values, i.e. the points in half below and half above the discriminatior.
...
A complication arises when an item is deleted from KD-tree when a node from the subtree must be replaced. This complication comes from the discriminator in that level. After the deletion, either the node with the smallest value in the right subtree should be replace or the node with the biggest value in the left subtree. Non-homogeneous KD-tree~\cite{nonhomkdtree} was proposed to make the process of deletion cheaper. The major difference of this structure is that the partitioning is not always on different dimensions cyclically. 
Instead, the dimension including more range queries are more desirable. 
As the name suggests, the partitioning in this tree can happen in an arbitrary hyperplane, a line in 2D,  in two different balanced subplances.

Another issue is the storage efficiency of the indexing.
...
KDB-tree has two basic structures: region page containing $(r, c)$ and point page containing $(p, l)$. A region page as an internal node contains a description of the bounding region $r$ enclosing its children and the reference $c$ to the children. 
%\todo the figure is not correct.
As shown in Figure~\ref{figkdbtree}, $C$, $D$, and $E$ subspaces are disjoint and together they shape region $A$ parent node.
A point page as a leaf node carries the object identifiers: the point $p$ and the location $l$.
Therefore, different pages are kept in different regions instead of the whole storage of the indexing tree. 
It should be mentioned that there is a trade off between being height-balanced and the storage efficiency.
Then it can happens that the storage efficiency become poorer in some cases. 
...
Since splitting the regions in KDB-tree results in sparse nodes at lower levels, HB-tree (holey brick B-tree)~\cite{hbtree1} as a multi-attribute index structure is proposed. HB-tree solves this prolem by allowing data spaces to be holey and consequently to be non-rectangular. 
It means the subspaces without data could be extracted from a region. To represent the holey regions, HB-tree uses KD-tree within internal nodes to organize the information about the lower levels. It might happen that a region is referenced by more than one leaf of KD-tree in internal nodes of HB-tree. In HB-tree leaf nodes are called \textit{data nodes} and internal nodes are \textit{index nodes}. 
%Index nodes  
...
Since KD-trees are designed as a point access method, its variant structure introduced by Matsuyama et al~\cite{matsuyama} is suitable for non-point objects via adding an extensive duplication strategy. In KD-tree, a data page is associated to each leaf nodes and reference to the objects totally or partially contained in it. Those objects which overlaps multiple data spaces are duplicated in all corresponding data pages. Then, the point is due to duplicated storage which means this structure is not appropriate for data with large objects.

Another interesting extension of KD-tree is 4D-tree~\cite{4dtree}.
In order to index two-dimensional objects, 4D-tree maps objects into four-dimensional points in KD-tree. It means a rectangle $(x‍‍_1, y_1)$, $(x_2, y_2)$ is considered as a point $(x‍‍_1, y_1, x_2, y_2)$. The discriminator is chosen repeatedly from the set $(x‍‍_1, y_1, x_2, y_2)$. 
For each node, a discriminator, a discriminator value and pointers to two children are stored. 
For a region search $(qx_1, qx_2, qy_1, qy_2)$, depending on the discriminator, the result of one of the operators,
$$x_1 \leq qx_2, \quad x_2 \geq qx_1, \quad y_1 \leq qy_2, \quad y^2 \geq qy^1,$$ 
