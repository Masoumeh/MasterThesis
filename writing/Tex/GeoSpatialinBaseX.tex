...
To see how different approaches based on the tree structres are included in these classes, spatial indexes are introduced briefly  based on three data structures Binary-tree, B-tree, and Quad-tree. We cover just a selective list since the number of these indexing methods
is a big number. Figure~\ref{trees} visualizes the relation of a part of indexing structures over years. An arrow from the structure $A$ to $B$ means $B$ is an extension of the structure $A$. The colored ovens also shows the main categories of these
trees. Quad-tree is represented as a subset of the space filling structures. Hashing structures are not in the focuse of this study, however they are used as a basis in the geospatial indexing of some databases. ?????????????Three following sections are a brief introduction of the tree important main categories:
...
The indexes in this group are basically and conceptually derived from the binary search tree. They adopt and generalize the idea of partitioning the space, as many sophisticated indexes. Here, we shortly explain some of them.

\textbf{The KD-tree}.
This index,first introduced by Bentley \cite{bently1975}, is a generalization of the binary tree structure for organizing $k$-dimensional points. As it is explained in Bentley \cite{bently1975}, the basic idea for a 2D example is to alternatively split the area by $x$- and $y$-coordinate such that the points are splitted at each level half in left, half in right, half below, and half above respectively. For every non-leaf node, there is a $k$-th dimension discriminator which defines the left and right subtrees. More clearly, if the discriminator is associated to the $i$-th dimension, all the $i$-th attribute of the sub-nodes in left subtrees are smaller than this node and all the right subtree nodes have the greater $i$-th attribute. Figure~\ref{figkdtree} shows an example of a data and the corresponding KD-tree. As it can be seen, according to the discriminator in first level, $x$, all nodes in the left subtree of $A$ have smaller $x$ value and all nodes in the right subtree have greater $x$ values. In the next level, the discriinator is $y$ value and all left subtree nodes have smaller $y$ value while the right subtree nodes have greater $y$ values.
...
\begin{itemize}
\item KDB-tree~\cite{kdbtree}: 
To solve the problem of the main memory storage, this tree takes advantage of B-tree IO-efficiency for optimizing external memory accesses together with the balanced structure of KD-tree. KDB-tree is also a space partitioning structure such that the partitions do not overlap each other. 
Space partitioning is similar to KD-tree. Unlike KD-tree, each divided space is not just its own node. Instead nodes are stored as pages. These partitions which stand for pages as organizational units, are organized in KD-tree structure. As shown in Figure~\ref{figkdbtree}, C, D, and E subspaces are disjoint and together they shape region A parent node.
\begin{figure}
\centering
...
\label{figkdbtree}
\end{figure}
KDB-tree has two basic structures: region page, consist of (region, pageID) pairs, and point page, consist of  (point, pageID) pairs. A region page as internal nodes contains the description of its subpages and a reference to those pages (see Figure~\ref{figkdbtree}) and point pages as leaf nodes only carry the object identifiers.
Different pages are kept in different regions instead of the whole storage of the indexing tree, called the paging technique~\cite{bintreepaging}. However, this technique enables the tree to be highly balanced, the storage efficiency is poorer. 
\item A non-homogeneous KD-tree: 
When an item is deleted from KD-tree, a node from the subtree must be replaced. Here arises a complication that based on the discriminator in that level, let's call it $i$, either the node with the smallest $i$ in right subtree should be replace or the node with the biggest $i$ in left subtree. Non-homogeneous KD-tree~\cite{nonhomkdtree} was proposed to make the process of deletion cheaper. The major difference of this structure is that the partitioning is not always on different dimensions cyclically. Instead, the dimension with more range queries on it are more desirable. As an alternative, splitting  longer dimensions yield more cubical spaces. In general, the dimension should be chosen such that the number of data entries in partionions are approximately equal in order to minimize the splitting process.

\item HB-tree: Since splitting the regions in KDB-tree results in sparse nodes at lower levels, HB-tree (holey brick B-tree)~\cite{hbtree1} as a multi-attribute index structure is proposed. HB-tree solves this prolem by allowing data spaces to be holey and consequently to be non-rectangular. It means data subspaces could be extracted from a region. To represent the holey regions, HB-tree uses KD-tree within internal nodes to organize the information about the lower levels. It might happen that a region is referenced by more than one lesf of KD-tree in internal nodes of HB-tree. In HB-tree leaf nodes are called \textit{data nodes} and internal nodes are \textit{index nodes}. Index nodes  
...
As~\cite{hbtree1} states, this structure by removing the sparse nodes of the KDB-tree, gives beter search and insert performance and decent space utilization. Nevertheless, expensive deletion and splitting are and multiple references to a data node that may lead to more than one traversal of a path are trade-off.

\item Matsuyama’s KD-tree:
Since KD-trees are designed as a point access method, its variant structure introduced by Matsuyama et al~\cite{matsuyama} is suitable for non-point objects via ading an extensive duplication strategy. In KD-tree, a data page is associated to each leaf nodes and reference to the objects totlly or partially contained in it. Those objects which overlaps multiple data spaces are duplicated in all corresponding data pages. The point is that due to duplicated storage this structure is not appropriate for large objects

\item 4D-tree~\cite{4dtree}: In order to index two-dimensional objects, ~\cite{4dtree} maps objects into four-dimensional points in KD-tree. It means a rectangle ($x‍‍_1$, $y_1$), ($x_2$, $y_2$) is considered as a point ($x‍‍_1$, $y_1$, $x_2$, $y_2$). The discriminator is chosen repeatedly from the set ($x‍‍_1$, $y_1$, $x_2$, $y_2$). For each node, a discriminator, discriminator value and pointers to two children are stored. 
For a region search ($qx_1$, $qx_2$, $qy_1$, $qy_2$), depending on the discriminator, the result of one of $x1 <= qx2, x2 >= qx1, y1 <= qy2, y^2 >= qy^1$ operations determines which subtree (or both) has to be searched.
...
\includegraphics[width=0.47\textwidth]{Rtree-box}
%\caption{R-tree structure}
\label{rtreebox}
}
\centering
\subfigure[R-tree structure]{
\includegraphics[width=0.47\textwidth]{Rtree}
%\caption{R-tree}
\label{rtreestruct}
}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.19\textheight]{R-treeMBR}
\caption{Two different split strategies in R-tree}
\label{rtreesplit}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{Rtree-query}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{r+tree}
...
{$R^+$-tree} might be bigger than R-Tree as a result of duplicated rectangle. Additionally, construction and maintenance are more complex.  
...
\caption{An Example of a cell tree}
\label{figcelltree}
\end{figure}
...
\textbf{X-tree}
X-tree, proposed by Berchtold et al~\cite{xtree}, is another extension to support high-dimensional data processing and focuses on prevention of MBRs overlaps. In this structure, if the split leads to large overlap, ...
