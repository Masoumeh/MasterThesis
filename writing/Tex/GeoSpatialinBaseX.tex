...
\label{s.disc}
Throughout this section we investigated different strategies to efficiently process the geospatial data in BaseX. Related to the most recent strategy in Section~\ref{MongoviaBX}, one might raise a question. The question could be, "Why not to choose the querying completely in MongoDB and to forget about querying MongoDB through BaseX, while MongoDB provides specific features more efficient?". To answer this question we start with a the point that a database might contain non-geospatial data in addition to the geospatial one. 
%Although BaseX can handle the non-geospatial queries as well as some geospatial queries like intersects %and within
%almost efficiently using Index, the near query and the database update decreases the efficiency dramatically. 
Therefore, besides the geospatial queries there might be some non-geospatial queries as well, e.g. finding the point elements or the number of polygons in a dataset. It means the geospatial queries are not necessarily the only focuse of the queries and the user can mix both types in a piece of code. Hence, these so-called non-geospatial queries should be considered in the set of user requiremets, however analysing them are not our focuse. This point is important when the user wants to choose between BaseX and MongoDB for geospatial queries while the non-geospaial queries will be done by BaseX. In this case the user can benefi from both databases while the most requirements are fulfilled by BaseX. 
%TODO:inja gosastegi ijad shode!!
To summerize the results observed outputs of the investigations, the near query and the geospatial index updating in BaseX are far from the ......???, though geospatial query times which are the focuse of the index structure, like intersects, seem that could be satisfying in some cases/some how.???  
Table~\ref{t.comparisonBaseXMongo} represents this statement based. \textit{MongoDB through BaseX} in this table means
BaseX connects to MongoDB to benefit by the features are not available or available with poor performance in BaseX, as discussed in Section~\ref{MongoviaBX}. The notation $"\times"$ in this table indicates better efficiency of that option. We suppose te user is satisfied with the above-mentioned non-geospatial queries.
...
\textbf{Tasks} & \textbf{BaseX} & \textbf{MongoDB through BaseX}\\\hline
 %Non-geospatial Queries & $\times$ &\\\hline
 Geospatial Queries  & $\times$ &\\
like intersects/within & & \\\hline
 Near Query & &$\times\times$ \\\hline
 DB modification  & &$\times\times$ \\
like update/insert/delete &&\\\hline
\end{tabular}
\caption{The comparison of two strategies using BaseX alone or connection to MongoDB}
\label{t.comparisonBaseXMongo}
\end{table}

Some fast conclusion (A fast conclusion???) from this table suggests the advantage of using BaseX through MongoDB. However, the issue of synchronization between the database instace of BaseX and the database instance of MongoDB should be precisely examined. If the user wants to modify the database which means also the modification of index, 
it seems that the faster strategy is to update the MongoDB instace, while the database instance in BaseX remains untouched. But, in case that the user wants to do further querying based on the changes, this approach do not seem optimized.  

A solution to this problem can be limiting the user such that they only do all queries regarding the index in MongoDB. ???
However, as we see in Table~\ref{t.comparisonBaseXMongo} some queries are more efficient in BaseX as the connection time to MongoDB is added. ???
Furthermore, the dependency of BaseX on MongoDB for all geospatial queries would be a big limitation for this software regarding the geospatial processing. Then, it will be more appropriate to provide the user with both possiblities.

Table~\ref{t.possibilities} summerizes the scenarios that can happen based on the existing features and investigations when the geospatial data wants to be processed. In addition, this table shows the relevant/appropriate actions the user can apply as an efficient strategy toward her/his goal.
...
and no update and no \textit{near} query (? near mitune bashe!) & \\\hline
...
\end{tabular}
\caption{Possible scenarios and suggested appropriate actions to take}
