...
\end{itemize}    
As Chin et al~\cite{survey} states, the SKD-tree directory is stored in secondary memory. Figure~\ref{figskdtree} illustrates the way that minimum and maximum boundaries of each space is determined to shape a virtual subspace. In this example, the discriminator for the root node is $x$ with the value of $x_6$.
...
Table~\ref{t.trees} summarizes the discussed indexing trees together with the advantages and disadvantages. 
...
HB-tree  & Handling sparsity & expensive deletion \\  
              & Better search performance & multiple reference \\
              & Better insert performance & to same data \\
...
R-tree  & Highly balanced point  & Large covered area  \\
        & and non-point objects  & and overlapping by MBRs \\
       & indexing               &  Multiple path traversal \\ 
        &                        & in point queries \\\hline
 \multicolumn{3}{c}{Based on the R-tree:}\\\hline
$R^+$-tree & No overlapping of MBRs  & Rectangle duplication\\
           & No multiple path traversal  &  Bigger structure\\
           &                             & Complicated structure\\
           &                             & Complex deletion process\\
           &                             & Dead spaces\\\hline
Cell-tree  & No overlapping and & Object duplication\\
          & no dead spaces      &        \\\hline
$R^*$-tree  & Minimizing coverage area,  & Slightly more cost\\
           &  overlapping,& than R-tree\\
            &  margin of MBRs, and& \\
...
STR-tree  & Only one time-consuming  & Poor performance \\
           &  preprocessing step       & in frequent modification\\
          &                      &   of data \\\hline
X-tree  & No overlapping of MBRs & Good performance\\\hline
