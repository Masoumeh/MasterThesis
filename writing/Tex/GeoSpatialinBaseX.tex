...
\label{s.disc}
Throughout this section we investigated different strategies to process the geospatial data in BaseX efficiently.
...
Table~\ref{t.comparisonBaseXMongo} summerizes this statement. As we discussed, \textit{BaseX through MongoDB} means
BaseX connects to MongoDB for the Indexing the geospatial data. Also, the results of queries will be
sent back to BaseX. The notation $\times$ in this table means that option is better in efficiency. 
With $\times\times$, we mean that option is very efficient.

\begin{table}
\centering
\begin{tabular}{|l | c | c|}\hline
\textbf{Tasks} & \textbf{BaseX} & \textbf{BaseX through MongoDB}\\\hline
...
Some fast conclusion from this table suggests the advantage of using BaseX through MongoDB. However,
the issue of synchronization between the database instace of BaseX and the database instance of MongoDB
should be precisely examined. If the user wants to modify the database which means also the modification of index, 
it seems that the better strategy is to update the MongoDB instace, while the database instance in BaseX remains unchanged.

This problem can be solved by limiting the user such that he can only do all queries regarding the index in MongoDB.
...
Also, the dependency of BaseX on MongoDB for all geospatial queries would be a big limitation for this software
regarding the geospatial processing. Then, both possiblities should be available for the user in our mind.

Table~\ref{t.possiblities} summerizes different scenarios that can be happened when 
a geospatial data wants to be processed. Also, this table shows the different actions 
that user can do for a efficient strategy toward her/his goal.
...
and no update and no \textit{near} query & \\\hline
 ...
 Needs a \textit{update/insert/delete} query& Using MongoDB only\\\hline
\end{tabular}
\caption{Different scenarios that can be happened and different actions that user can take}
