...
As shown in Figure~\ref{figkdbtree}, $D$, $E$, $F$, and $G$ subspaces are disjoint and together they shape region $A$ parent node.
...
Since splitting the regions in KDB-tree results in sparse nodes at lower levels, HB-tree (holey brick B-tree)~\cite{hbtree1} as a multi-attribute index structure is proposed to solve this prolem. HB-tree allows data spaces to be divided along more than one dimension. It permits rectagular holes inside a data space  by partitioning along pieces of several dimensions. This gives more flexibility in patitioning and serves as a solution for the cases where partitioning a region along one entire dimension forces splitting of the children subregion. Also, this structure helps to distribute the objects in subspaces more evenly when it is ot possible by cutting across one dimension. This point is illustrated in Figure~\ref{hbtree}.

\begin{figure}
\centering
\subfigure[Single-dimensional split of data points]{
\includegraphics[width=0.22\textwidth,height=0.13\textheight]{hbtree1}
%\caption{R-tree structure}
\label{hbsplita}
}
\centering
\subfigure[Two-dimensional split of data points]{
\includegraphics[width=0.22\textwidth,height=0.13\textheight]{hbtree2}
%\caption{R-tree}
\label{hbsplitb}
}
\caption{Partitioning of data space in HB-tree}
\label{hbtree}
\end{figure}

To represent the holey regions, HB-tree uses KD-tree within internal nodes to organize the information about the lower levels. It means that more complex data structure is required to represent a node. In addition, it might happen that a region is referenced by more than one leaf of KD-tree in internal nodes of HB-tree (see Figure~\ref{fighbtree}). This implies that this structure is not exactly a tree. 
...
Since KD-trees are designed as a point access method, the variant structure introduced by Matsuyama et al~\cite{matsuyama} is suitable for non-point objects via adding an extensive duplication strategy. In KD-tree, a data page is associated to each leaf nodes and reference to the objects totally or partially contained in it. Those objects which overlaps multiple data spaces are duplicated in all corresponding data pages. Then, the point is due to duplicated storage which means this structure is not appropriate for data with large objects.

Another extension of KD-tree to make it useful to index two-dimensional objects is 4D-tree~\cite{4dtree}.
4D-tree maps objects into four-dimensional points in KD-tree. It means a rectangle $(x‍‍_1, y_1)$, $(x_2, y_2)$ is considered as a point $(x‍‍_1, y_1, x_2, y_2)$. The discriminator is chosen repeatedly from the set $(x‍‍_1, y_1, x_2, y_2)$. 
