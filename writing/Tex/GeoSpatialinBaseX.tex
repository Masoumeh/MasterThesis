...
%There are two advantages to consider this tree. First, this tree is efficient when the data is almost fixed which means the data has minor changed after importing. Second, the JTS library which we use as the basic for Geometry operation, already includes this tree as one of its indexing trees.
This structure which is included in JTS library, the API used in BaseX implementing Geo Module, is suitable to focus on the data with minor changes and improves the intersection queries. %There were several reasons:
%\begin{itemize}
