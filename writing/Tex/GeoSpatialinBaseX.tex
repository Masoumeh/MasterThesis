...
Finally, we discuss few interesting approaches in Section~\ref{others}.
...
is not applicable to spatial data, as it is discussed in~\cite{Lisa2006,Chen2010}. According to the capability of this language, high number of 
...
to achieve a GML query processor. New spatial datatypes and operators are added to XQuery to support both spatial and mix (spatial and non-spatial) queries. Unlike the previously mentioned approach in Chen~\cite{Chen2010}, supported data types are limited and predefined to Geometry, Coord, Coordinates, Point, LineString, LinearRing, Polygon, Box, GeometryCollection, MultiPoint, MultiLineString, and MultiPolygon. 
The spatial operations defined in this method are as follows,
\begin{itemize}
...
which is not based on the tree-based (syntactic) structure of GML. Instead, a system is developed to implement a query language based on the semantic structure of GML and the GML schema is used to define the queries. However, a semantic content can be structured in several syntaxes. The system stores GML by PostGIS database system and transforms the XPath-based GML queries into the SQL language, considering the GML schema inorder to execute semantic based queries and at the end visualizes the results exported into KML format.
...
\label{others}
In addition to the forementioned topics, some other areas have been discovered as well in this reseach line. Web technology, being widely used nowadayas, is an area discoverd by Corcoles~\cite{corcoles2003}. This effort proposes an approach to integrate geospatial data 
...
%These databases are selected since they are leading databases in the field of XML data processing and
all of them have geospatial functionalities.
%We give a brief introduction of the geospatial processing in this databases in the following sections.
