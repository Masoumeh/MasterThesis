...
in a XQuery\cite{xquery}-like language with spatial functional extensions. These GML queries are translated into the equivalent SQL queries which are evaluated using the spatial database management system. 
A similar approach is discussed in \cite{Zhu2011} to store and query the GML data focusing on non-schema documents. After the generation of GML parse tree, the nodes are analyzed and schema mapping is generated to store the document in an object-relational database. 
Both spatial and non-spatial queries are supported in this approach.

In constrast to these two approaches, Shulian Zhang et al~\cite{Zhang2008} considers the characteristics of XML databases and stores the GML data in the original format without any conversion or mapping to a relational or object-orineted database. 
It supports also the self-descriptive and semi-structural characteristics of XML.
This is only one of the strategies which propose a native XML database, considering the GML structure in storage and querying. The corresponding strategy based on the~\cite{Zhang2008} and other similar works follows the below steps:
\begin{enumerate}
...
Another challenging issue in storage is the size of GML data that is commonly huge. We bring one example of several research papers in this area to have a complete discussion over the storage.
...
Finally, we discuss other nonrelated approaches in Section~\ref{others}.
...
JTS API component is implemented in this engine
...
The defined spatial operations are simple geometry operation, 
spatial relation operation, spatial analysis operations, and geometry-specific operations.
...
\label{mongo}
MongoDB~\cite{mongogeneral2010,mongoinaction2011} supports both flat and spherical space in geospatial processings. 
...
Here, we briefly describe the general ideas of geospatial support to see the properties and implementation model of the functions and the index structure. 
\footnote{The Spatial Module in eXist-db is still in experimental satus.}
