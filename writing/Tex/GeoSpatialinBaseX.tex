...
\newpage
\section*{Abstract}
Geospatial information has been growing expotentionally and used in variety of applications such as GIS, bioinformatics, and decision support systems during the last decades. The number of applications are becoming more such that efficient  approaches for data manupulation are demanded to cover the requirements. XML-based data representation and manipulations is one of the approaches that has been developed and can be processed by native XML databases. To see how this process could be challenging in an XML database, this thesis represents the issues of geospatial querying, functionalities and more importantly query efficiency by following one of the possible ways in BaseX, as an open source native XML database. First, a number of indexing approaches are studied to compare various ways of efficient geospatial data manipulation. Second, general geospatial functionalities in some similar databases are investigated. Then, the geospatial functionalities are added to BaseX by using the widely used JTS library. To improve the efficiency in some queries, the included index structure in JTS is applied and the query timing improvment is evaluated. We have tried another querying approach, as an effort to find other efficient ways, by connection to a well-known database, named MongoDB, through BaseX and having the queries done. The evaluation of this approach shows improvements in update operations which can be considered in deciding on the future developments in BaseX.
...
\setcounter{page}{1}
Geospatial data supports a wide range of ever-expanding applications in various scientific, govermental, business, and industrial areas such as geoscience, energy exploration, environmental management, natural resource management, disaster management, transportation engineering, urban planning and design, and constructions. Numerous researchs and efforts have been dedicated to make the most efficient use of this data, either in representation or manipulation. XML-based structures are part of the wiedly used standards to represent the complex structure of geospatial data. GISs, traditional databases, and other systems have been developed or adopted to handle this type of data format. Storage and retrieval of geospatial data which benefits the XML advantages in native XML databases could also be a suitable way to fulfill the geospatial requirements. In this way, the first step is to have geospatial functionalities in the database which is the main focuse of this work.
%main focuse of the current work is to add geospatial features in BaseX while investigating an insight some similar database systems in this topic. 
However, performance improvement in geospatial data retrieval is a big challenge since this data is often huge in size and complex in structure. Numerous spatial indexing structures propose suitable ways for efficient querying according to specific requirements and focus. 

The aim of this thesis is to introduce ways to empower BaseX to process geospatial data in an efficient way. To achieve this goal, the support of handling \emph{Geography Markup Language (GML)} data. as one of the widely use XML-based standards, together with an indexing structure is developed and evaluated in BaseX. Meanwhile, some other indexing structures and geospatail features in similar database systems is studied which is ofcourse an introductory point to find the most proper approach in providing geospatial functionalites in BaseX. As an experimental way to provide some efficient geospatial features, we make a connection to MongoDB, a powerful document-based database, and run some queries. The results and evaluations can arise some suggestions for further investigations and developments in BaseX. 
%time complexity is specially of great interest. 
...
\subsection{Overview}
This thesis will first define the concepts needed later in the discussion, called
...
In this section, we mainly explain some existing geospatial indexing structures as the most critical part of geospatial data processing in database systems. After some basic definitions in Section \ref{termsanddef}, general view of the geospatial data processing in XML databases is shortly discussed in Section \ref{s.geo-data-in-xml-data}.
...
%For example, WGS coordinate system is defined to handle the geographic data on a map or on the Earth. 

A country may have more than one coordinate system for its national-wide geographic data. As an instance, all three systems \textit{DHDN}\footnote{Deutsches Hauptdreiecksnetz}, \textit{ETRS89 UTM zone 32N}, and \textit{PD/83 Zone 3} are examples of Germany's coordinate systems. These systems are considered as local or regional ones while there are the global coordinate systems to give features of the world unique coordinates and specifications, like WGS system. The coordinates of one feature or object may differ from one system to another. As an example, the coordinates of Berlin in WGS84 coordinate system with longitude and latitude is $(52.520007, 13.404954)$ while in Germany's \textit{ETRS89 UTM zone 32N} coordinate system is $(32798809.64, 5828000.49)$. Accordingly, it is necessary to know how the coordinate systems are related in concepts and definitions to make the conversion from one system to another possible. The conversion is required when we need o move data between coordinate systems.
...
\begin{itemize}
\item Global or spherical coordinate system: 

Also referred as \textit{geographic coordinate system}, represents the data on the Earth in a 3D spherical surface by longitudes and latitudes. Points are referenced by their longitude and latitude values which are are angles measured often in degrees (or in grads) from the earth's center to a point on the earth's surface. Sometimes it is useful to associate Longitude and latitude values with $x$ and $y$ axis respectively. WGS84, NAD\footnote{North American Datum} 1927, and NAD 1983 are examples of geographic coordinate systems. 

\item Projected coordinate system: 

This coordinate system represents the Earth's round surface on a 2D flat surface, based on a map projection as shown in Figure~\ref{figproject}. In fact, a projected coordinate system is always based on a geographic coordinate system on Earth and the word "projected" referes to projection of the data on the spherical Earth to a flat space, called \textit{map projection}. Points in such system are located by $x$ and $y$ values on a grid. 
%Each projection distorts the data in some way on the Earth. It is important to know about the distortion of a particular projection, i.e. between two specific coordinate system, when selecting it for the data projection. 
...
To define a coordinate system, some parameters should be considered, such as the measurement framework (geographic  or planimetric), the unit of measurement (e.g. meter for projected coordinate systems and decimal degrees for geosraphical coordinate systems), and the projection definition for projected coordinate system.
%geographic (measuring spherical coordinates from the Earth's center) or planimetric (projecting the Earth's coordinates onto a two-dimensional planar surface)
...
\begin{itemize}
\item Object Mapping: Given a $k$-dimensional polygon with $n$ nodes, these approaches map this shape to a $kn$-dimensional or try to look at the shape of a polygon as a single object in the original $k$-dimensional polygon.   
...
A complication arises when an item is deleted from KD-tree and a node from the subtrees must be replaced. This complication comes from the discriminator in that level. After the deletion, either the node with the smallest value in the right subtree should be replaced or the node with the biggest value in the left subtree. 
...
Since the size of indexing can grow fast enough to fill a huge chunk of main memory, most of the index have to be stored necessarily on disk while being used.
To make efficient use of the secondary memory, KDB-tree~\cite{kdbtree} is proposed to provide the search efficiency of a balanced KD-tree together with the optimized storage access taken from the B-tree. It partitions the areas similar to KD-tree and stores each node as a page similar to the B-tree. Pages exist in two types, 
...
region and point pages. A region page as an internal node contains a description of the bounding region enclosing its children and the reference to the children. 
...
As a property of the KDB-tree structure, the regions in every region page are disjoint and their union is the parent region. Moreover, the union of regions in a child region page is the parent region. Similarly, all points in a point page as a child of a region are necessarily in the region.
...
As Robinson~\cite{kdbtree} states, KDB-tree give more efficient search structure for large multi-dimensional dynamic structures with optimized storage utilization. It should be mentioned that there is a trade-off between being height-balanced and the storage efficiency. Then, the storage efficiency can be even poorer in some cases. ?????
...
\textbf{HB-tree.} 
Splitting the region nodes is inevitable for keeping the balance of KDB-tree during the modification processes. This splitting is done by intersecting the data space with a plane. This intersection does not always cross the actual borders of the current subregions. Then, new subspaces would be produced. This means splitting can propagate to the children, resulting in more and more sparse nodes in lower levels. HB-tree (holey brick B-tree)~\cite{hbtree1} as a multi-attribute index structure is proposed to solve this prolem. HB-tree allows data spaces to be divided along more than one dimension that can shape rectangular regions inside that space, called holes.
%It permits rectagular holes inside a data space by partitioning along pieces of several dimensions. 
This gives more flexibility in patitioning and serves as a solution for the cases where partitioning a region along one entire dimension forces splitting of the children subregion. 
...
To represent the holey regions, HB-tree uses KD-tree within internal nodes to organize the information about the lower levels. It means that more complex data structure is required to represent a node. In addition, it might happen that a region is referenced by more than one leaf of KD-tree in internal nodes of HB-tree, such as node $G$ in Figure~\ref{fighbtree}. As it can be seen in this Figure, splitting along $y$ in third level results in dividing into two subtrees, one having $1/3$ and the other $2/3$ of the nodes. To achieve this, the subtree with node $x_2$ shows the splitt along $x$ in the next level and therefore duplicates $G$ in both right and left subtrees. The subtrees with root noeds $x_1$, $x_2$, $y_3$, and $y_5$ are extracted to shows $F$ and $C$ regions.
...
As Lomet and Salzberg~\cite{hbtree1} states, removing the sparse nodes of the KDB-tree, gives better search and insert performance and decent space utilization in HB-tree. Nevertheless, expensive deletion and splitting as a result of complex structure, and multiple references to a data node that may lead to more than one traversal of a path are trade-off.
...
are basically the same as in B-tree. The only difference is that the 
...
With the changes made in the structure, {$R^+$-tree} is at some advantages. Point queries performance benefits from the abscence of overlappings and instead of several paths, a single path is traversed. Consequently fewer nodes are visited in point queries. On the other hand, there are some disadvantages. 
...
\subsubsection{Quad-tree}???? raazi nistam! :P
...
Among this category, we explain here the Quad-tree~\cite{quadtree} briefly as the hierarchial spatial data structure?????(cheraaaa). Quad-trees are based on the principle of recursive decomposition. The goal of this tree is to design a structure in main memory and often for the two-dimensional data. 
...
\subsection{Summary}
Regarding the discussion in this section, there is no optimum indexing structure for any type of data and requirements. Based on the application, each structure has own pros and cons. We explained some important indexing trees which were introduced mostly in Chin et al~\cite{survey} and Brown et al~\cite{tree-basedindexes}. Also, we looked at the actual paper of each tree to gather the advantages and disadvantages. For sure, we did not want to have an extensive research on these indexing trees since it is not our focuse. But we want to have at least an introduction about the huge area of spatial indexing. 
Table~\ref{t.trees} summerizes the discussed indexing trees together with the advantages and disdvantages. 
Among the presented trees, STR-tree is the one which we have selected for our further implementation. There were several reasons:
\begin{itemize}
...
\hline
\textbf{The indexing tree} & \textbf{Based on} & \textbf{Advantage} & \textbf{Disadvantage} \\\hline
...
R-tree & B-tree & Highly balanced point  & Large covered area  \\
           &           &                                      & and overlappings by MBRs \\\hline
           &          &  and non-point objects indexing    &  Multiple path traversal \\ 
           &           &                                                      & in point queries \\\hline
$R^+$-tree & R-tree & No overlappings of MBRs  & Rectangle duplication\\
                     &           & No multiple path traversal  &  Bigger and complicated structure\\
                     &           &                                            & Complex deletion process, Dead spaces\\\hline
Cell-tree & R-tree & No overlappings and dead spaces & Object duplication\\\hline
$R^*$-tree & R-tree & Minimizing the coverage area, overlappings  & Silghtly more cost than R-tree\\
                    &           &  and margin of MBRs                                     & \\
                    &           & Minimizing the storage utilization                  &\\\hline
Packed-tree & R-tree & Only one time-consuming preprocessing step & Poor performance \\
               &           & Storage and performance efficiency                & in frequent modification of data\\\hline
STR-tree & R-tree & Only one time-consuming preprocessing step & Poor performance \\
               &           & Packed in more dimensions                            & in frequent modification of data\\\hline
X-tree & R-tree & No overlappings of MBRs & Good performance\\\hline
Quad-tree & Space filing structures & Efficient for sparse data & Poor performance \\
