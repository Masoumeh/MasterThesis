...
\begin{titlepage}
\begin{center}
{\LARGE \textbf{Geospatial Processing in BaseX}}
\\[1cm]
{\normalsize {Master Thesis in fulfillment of the requirements \\
for the degree of Master of Science (M.Sc.)}}
\\[1cm]
{\small submitted from}
\\[0.5cm]
...
\\[1cm]
{\normalsize {Department of Computer and Information Science}}
\\
{\normalsize {Chair of Database and Information Systems Group}}
\\[2cm]
\begin{minipage}[c]{0.8\textwidth}
\begin{description}[style=multiline]
 \item {\large \textbf{1.Supervisor:} Prof. Dr. Marc H. Scholl}
 \item {\large \textbf{2.Supervisor:} Prof. Dr. M. Grossniklaus}
\end{description}
\end{minipage}
\vfill
...
I should confess that I cannot imagine a better person to guide and advise me. Besides, I would like to extend my gratitude to Prof. Dr. Marc H. Scholl to give me the chance to be involved in this group and to Prof. Dr. Michael Grossniklaus for his helpful guidance and comments.
...
Native XML database is also another approach introduced in recent years to fulfill the geospatial requirements. This requires adding geospatial functionality as a starting point.
...
\includegraphics[width=0.6\textwidth,height=0.2\textheight]{skdtree2}
%\caption{}
\label{skdtree2}
}
\centering
\subfigure[A 2D directory of the SKD-tree]{
\includegraphics[width=0.6\textwidth,height=0.2\textheight]{skdtree1}
%\caption{}
\label{skdtree1}
}
...
\vspace{10px}
\begin{fakeXML}[label=qlang1,caption=A sample query of a flexible query language.]
...
was an area investigated by C{\'o}rcoles~\cite{corcoles2003} for the geospatial data. 
...
C{\'o}rcoles~\cite{corcoles2003} discusses a prototype of an integration mediator for querying the spatial XML resources. 
...
\vspace{10px}
\begin{fakeXML}[label=geoandfilter,caption=The geo function used together with the index function]
import module namespace geo-index = "http://expath.org/ns/GeoIndex";
...
\vspace{10px}
\begin{fakeXML}[label=basex-query,caption=Example query of the \textit{intersects} function in BaseX]
import module namespace geo = "http://expath.org/ns/Geo";
declare namespace gml="http://www.opengis.net/gml";
let $a:= <gml:Polygon>
              <gml:outerBoundaryIs>
...
\end{fakeXML}
\vspace{10px}
\begin{fakeXML}[label=mongo-query,caption=Example query of the \textit{intersects} function in MongoDB]
...
%\caption{Query time of \textit{intersect} operation without geospatial index}
...
\vspace{10px}
\begin{fakeXML}[label=nearDistance,caption=Sample query using \textit{\$nearSphere} in MongoDB]
...
\vspace{10px}
\begin{fakeXML}[label=nearBX, caption=Sample query to the \textit{near} feature in BaseX]
import module namespace geo = "http://expath.org/ns/Geo";
declare namespace gml="http://www.opengis.net/gml";
let $a:= <gml:Point>
              <gml:coordinates>
...
Table~\ref{t.comparisonBaseXMongo} summarizes the statement above. \textit{MongoDB through BaseX} in this table means that BaseX connects to MongoDB to benefit by the features which are not available or available with poor performance in BaseX, as discussed in Section~\ref{MongoviaBX}. \textit{GML-related} features points the aforementioned features which are supposed here to be satisfying enough for the user, indicated with the notation $``\Plus"$. This assumption is made in order to concentrate on the evaluation of those cases in which the user choose to use BaseX and we try to cover the missing geospatial features.
%Otherwise, the user might choose to work with another system, like MongoDB.
% is better in efficiency.
With $``\Plus\Plus"$, we mean that option is very efficient.
\begin{table}
\centering
\begin{tabular}{|l | c | c|}\hline
\textbf{Tasks} & \textbf{BaseX} & \textbf{MongoDB through BaseX}\\\hline
 \textit{GML-related} Queries & $\Plus$ &\\\hline
 Geospatial Queries,  & $\Plus$ &\\
like \textit{intersects/within} & & \\\hline
 The \textit{near} Query & &$\Plus\Plus$ \\\hline
 DB Update Queries & &$\Plus\Plus$ 
...
Table~\ref{t.possibilities} summarizes the sample scenarios that could happen based on the existing features to process the geospatial data. In addition, this table shows the relevant and appropriate actions the user can apply as an efficient strategy toward her\-/his goal.
