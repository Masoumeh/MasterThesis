...
\newpage
\section*{Abstract}
Geospatial information has been growing expotentially and is used in variety of applications such as GIS, bioinformatics, and decision support systems during the last decades. 
The varierty of applications demanded efficient approaches for data manupulation to cover their requirements. 
The XML-based representation and manipulation of data is one of the approaches that has been developed recently. 
Numerous native XML databases are implemented to handle this data. 
The open-source software BaseX is one of these native XML databases. 
The contribution of the current work is to see how the geospatial data process could be challenging in BaseX. 
This thesis discussess the issues of geospatial querying, geospatial functionalities, and query efficiency by following the existing structure in BaseX. 
First, a number of indexing approaches are studied to compare various ways of efficient geospatial data manipulation. 
Second, general geospatial functionalities in similar databases are investigated. 
Finally, the geospatial functionalities are added to BaseX based on GML standard.
The geospatial query is adpated also in the structure of BaseX. 
To improve the query efficiency, the data is indexed by the indexing structure STR-tree.  
We have tried another querying approach by connecting to MongoDB through BaseX as an effort to find more efficient ways.
We did evaluation in different stages and concluded advantages and disadvantages of different approaches. 
\thispagestyle{empty}
...
XML-based structures are parts of the wiedly used standards to represent the complex structure of geospatial data. 
...
However, performance improvement in geospatial data retrieval is a big challenge since this data is often huge in size and complex in structure. 
Spatial indexing structures are studied to find suitable ways for efficient querying according to specific requirements. 

BaseX~\cite{www/basex} is a high-performance XML~\cite{www/xml} database which is developed in Konstanz university.
...
This means to add the support of handling \emph{Geography Markup Language (GML)} data. 
GML is a soecuak XML grammer for the geographical data which is defined by the Open Geospatial Consortium (OGC)
recognized internationally for geospatial standards.
Having the geospatial data in BaseX, the query can be done over them.
Meanwhile, some other indexing structures and geospatail features in similar database systems is studied which is an introductory point 
to find the most proper approach in providing geospatial functionalites in BaseX. 
As an experimental way to provide more efficient geospatial features, 
we make a connection to MongoDB, a powerful document-based database, 
and run some queries. The results and evaluations arise some suggestions for further investigations and developments in BaseX. 
...
%\subsection{Overview}
This thesis is structred as follows. We first define the concepts needed later in the discussion
in Section~\ref{s.method}. In this section, the basic geospatial data
...
The indexing tree is stored in the main memory which results in the high reading time.
One of the first idea is to bring some parts of the tree, which are addresses more,
...
In this thesis, we consider MongoDB as an alternative for indexing.
A feasible improvement would be the implementation of an flexible interface
to this XML database. As we discussed in Section~\ref{conc}, 
there would be an inconcsistency between 
the geospatial data and indexing in BaseX and
the simultanaesly indexed data in MongoDB. A future improvement is to 
